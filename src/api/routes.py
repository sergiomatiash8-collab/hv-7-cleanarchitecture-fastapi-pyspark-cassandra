...
def get_top_products(
    n: int = 10, 
    period: str = "2005-09", 
    use_case: GetTopReviewedProductsUseCase = Depends(get_top_products_use_case)
):
    # Змінено з start/end на period, щоб відповідати Partition Key у Cassandra
    return use_case.execute(n, period)

@router.get("/top-customers")
def get_top_customers(
    n: int = 10, 
    period: str = "2005-09", 
    use_case: GetTopCustomersUseCase = Depends(get_top_customers_use_case)
):
    return use_case.execute(n, period)
...
def get_top_haters(
    n: int = 10, 
    period: str = "2005-09", 
    use_case: GetTopHatersUseCase = Depends(get_top_haters_use_case)
):
    return use_case.execute(n, period)
...
def get_top_backers(
    n: int = 10, 
    period: str = "2005-09", 
    use_case: GetTopBackersUseCase = Depends(get_top_backers_use_case)
):
    return use_case.execute(n, period)